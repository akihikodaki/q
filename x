#!/usr/bin/env ruby
# frozen_string_literal: true

require 'etc'

mkosi = if Etc.nprocessors < 18
          [
            "--kernel-command-line-extra=isolcpus=#{Etc.nprocessors}-18",
            '--qemu-smp=18,sockets=2'
          ]
        else
          %W[--qemu-smp=#{Etc.nprocessors},sockets=2]
        end

qemu = %W[
  -device virtio-iommu
  -device pcie-root-port,id=port0,slot=0
  -device pcie-root-port,id=port1,slot=1
  -device pcie-root-port,id=port2,slot=2
  -device pcie-root-port,id=port3,slot=3
  -device pcie-root-port,id=port4,slot=4
  -device pcie-root-port,id=port5,slot=5
  -device pcie-root-port,id=port6,slot=6
  -device pcie-root-port,id=port7,slot=7
  -device virtio-net-pci,bus=port0,mac=02:00:00:01:00:00,netdev=hub0port0
  -device #{ARGV[0]},bus=port1,mac=02:00:00:01:00:01,netdev=hub0port1
  -device virtio-net-pci,bus=port2,mac=02:00:00:01:01:00,netdev=hub1port0
  -device #{ARGV[0]},bus=port3,mac=02:00:00:01:01:01,netdev=hub1port1
  -device virtio-net-pci,bus=port4,mac=02:00:00:01:02:00,netdev=hub2port0
  -device #{ARGV[0]},bus=port5,mac=02:00:00:01:02:01,netdev=hub2port1
  -device virtio-net-pci,bus=port6,mac=02:00:00:01:03:00,netdev=hub3port0
  -device #{ARGV[0]},bus=port7,mac=02:00:00:01:03:01,netdev=hub3port1
  -netdev hubport,id=hub0port0,hubid=0 -netdev hubport,id=hub0port1,hubid=0
  -netdev hubport,id=hub1port0,hubid=1 -netdev hubport,id=hub1port1,hubid=1
  -netdev hubport,id=hub2port0,hubid=2 -netdev hubport,id=hub2port1,hubid=2
  -netdev hubport,id=hub3port0,hubid=3 -netdev hubport,id=hub3port1,hubid=3
  -numa node,memdev=ram0 -object memory-backend-ram,size=7G,id=ram0
  -numa node,memdev=ram1 -object memory-backend-ram,size=7G,id=ram1
  -numa cpu,node-id=0,socket-id=0 -numa cpu,node-id=1,socket-id=1
  -virtfs local,path=#{__dir__},mount_tag=q,security_model=none
]

exec({ 'PATH' => "#{__dir__}/var/qemu/build:#{ENV['PATH']}" },
     'mkosi', *mkosi, 'qemu', *qemu, *ARGV[1..])
